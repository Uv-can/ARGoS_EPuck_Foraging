#include "epuck_foraging_loop_functions.h"
...
CEPuckForagingLoopFunctions::CEPuckForagingLoopFunctions() :
...
void CEPuckForagingLoopFunctions::Init(TConfigurationNode& t_node) {
   try {
...
void CEPuckForagingLoopFunctions::Reset() {
   /* Zero the counters */
   m_unCollectedFood = 0;
   m_nEnergy = 0;
   /* Close the file */
...
void CEPuckForagingLoopFunctions::Destroy() {
   /* Close the file */
   m_cOutput.close();
}
...
CColor CEPuckForagingLoopFunctions::GetFloorColor(const CVector2& c_position_on_plane) {
...
void CEPuckForagingLoopFunctions::PreStep() {
   ...
   for(CSpace::TMapPerType::iterator it = m_cEPucks.begin();
      ...
      /* Get the position of the e-pucks on the ground as a CVector2 */
      CVector2 cPos;
      ...
      if(sFoodData.HasFoodItem) {
         ...
         /* The e-puck has no food item */
         /* Check whether the e-puck is out of the nest */
         if(cPos.GetX() > -1.0f) {
            /* Check whether the e-puck is on a food item */
            bool bDone = false;
            for(size_t i = 0; i < m_cFoodPos.size() && !bDone; ++i) {
               if((cPos - m_cFoodPos[i]).SquareLength() < m_fFoodSquareRadius) {
                  /* If so, we move that item out of sight */
                  m_cFoodPos[i].Set(100.0f, 100.f);
                  /* The e-puck is now carrying an item */
...
REGISTER_LOOP_FUNCTIONS(CEPuckForagingLoopFunctions, "epuck_foraging_loop_functions")
