...
CEPuckForaging::SDiffusionParams::SDiffusionParams() :
   GoStraightAngleRange(CRadians(-1.0f), CRadians(1.0f)) {}

void CEPuckForaging::SDiffusionParams::Init(TConfigurationNode& t_node) {
   try {
...
void CEPuckForaging::Init(TConfigurationNode& t_node) {
   try {
      ...
      m_pcRABA      = GetActuator<CCI_EPuckRangeAndBearingActuator     >("range_and_bearing"    );
      m_pcRABS      = GetSensor  <CCI_EPuckRangeAndBearingSensor       >("range_and_bearing"    );
      m_pcProximity = GetSensor  <CCI_EPuckProximitySensor      >("proximity"    );
      m_pcLight     = GetSensor  <CCI_EPuckLightSensor          >("light"        );
      m_pcGround    = GetSensor  <CCI_EPuckGroundSensor    >("ground" );
      /*
      ...
      THROW_ARGOSEXCEPTION_NESTED("Error initializing the epuck foraging controller for robot \"" << GetId() << "\"", ex);
   }
...
void CEPuckForaging::UpdateState() {
   ...
   const CCI_EPuckGroundSensor::SReadings& tGroundReads = m_pcGround->GetReadings();
        ...
        if(tGroundReads.Left > 0.25f &&
      tGroundReads.Left < 0.75f) {
...
void CEPuckForaging::SetWheelSpeedsFromVector(const CVector2& c_heading) {
   ...
   void CEPuckForaging::Rest() {
      ...
      if(m_sStateData.TimeRested > m_sStateData.MinimumRestingTime &&
         ...
         const CCI_EPuckRangeAndBearingSensor::TReadings& tPackets = m_pcRABS->GetReadings();
